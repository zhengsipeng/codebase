...
import torchvision
if float(torchvision.__version__[:3]) < 0.7 and torchvision.__version__[:4] not in ["0.10", "0.11", "0.12"]:
...
def collate_fn(batch, fix_input=None):
    batch = list(zip(*batch))
    if len(batch) > 2:
        batch[0] = nested_tensor_from_tensor_list(batch[0] + batch[1], batch[2] + batch[3], fix_input=fix_input)
        new_batch = tuple([batch[0], batch[2] + batch[3]])
        #return tuple([batch[0], batch[2] + batch[3]])
    else:
        batch[0] = nested_tensor_from_tensor_list(batch[0], batch[1], fix_input=fix_input)
        new_batch = tuple(batch)
        #return tuple(batch)
    
    if "input_seq" in new_batch[1][0]:
        output_len = max([item['target_seq'].shape[0] for item in new_batch[1]])
        input_seqs = torch.zeros((len(new_batch[1]), output_len-1), dtype=torch.long)
        output_seqs = torch.zeros((len(new_batch[1]), output_len), dtype=torch.long) - 100
        for idx, item in enumerate(new_batch[1]):
            i_seq, o_seq = item["input_seq"], item["target_seq"]
            input_seqs[idx, :i_seq.shape[0]] = i_seq
            output_seqs[idx, :o_seq.shape[0]] = o_seq
        
        return tuple([new_batch[0], input_seqs, output_seqs, new_batch[1]])
    else:
        return new_batch
