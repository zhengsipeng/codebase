...
from src.models.tsg.eval import iou
...
class ActNetDataset(Dataset):
    ...
    def __init__(self, config, split, metadata_path, video_path):
        super(ActNetDataset, self).__init__()
        self.cfg = config
        self.vis_input_type = config.DATA.VIS_INPUT_TYPE
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.DATA.NUM_SAMPLE_CLIPS > 0:
            # visual_input = sample_to_fixed_length(visual_input, random_sampling=True)
            visual_input = average_to_fixed_length(visual_input, self.cfg)
            num_clips = self.cfg.DATA.NUM_SAMPLE_CLIPS//self.cfg.DATA.TARGET_STRIDE
        ...
        # Time unscaled NEED FIXED WINDOW SIZE
            ...
            num_clips = visual_input.shape[0]//self.cfg.DATA.TARGET_STRIDE
    ...
    def get_video_features(self, vid):
        assert self.cfg.DATA.VIS_INPUT_TYPE == 'c3d'
