...
def main_Homage(dataset, v_root, save_root):
...
def append_missing_imgs(dbname):
    if dbname == "vidvrd":
        anno_files = glob.glob("/dataset/28d47491/zsp/data/%s/annotation/**/*.json"%dbname)
    else:
        anno_files = glob.glob("/dataset/28d47491/zsp/data/%s/annotation/***/**/*.json"%dbname)
    ...
    for anno_file in tqdm(anno_files):
        with open(anno_file, "r") as f:
            anno = json.load(f)
        num_frames = anno['frame_count']
        video_id = anno["video_id"]
        subdir = anno['video_path'].split("/")[0]
        if dbname == "vidvrd":
            image_dir = "/dataset/28d47491/zsp/data/%s/images/%s"%(dbname, video_id)
        else:
            image_dir = "/dataset/28d47491/zsp/data/%s/images/%s/%s"%(dbname, subdir, video_id)
...
if __name__ == '__main__':
    ...
    #main_Homage(args.dataset, v_root=args.vpath, save_root=args.savepath)

    append_missing_imgs(args.dataset)

# python extract_frame.py --dataset vidor --vpath /data3/zsp/data/vidor/video --savepath /data3/zsp/data/vidor/images